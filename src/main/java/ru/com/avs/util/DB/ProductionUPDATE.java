...
public class ProductionUPDATE {
    ...
    public void init() throws SQLException {
        exec  = new Executor("jdbc:derby:scales-rest-service-db", "avs", "evbhPoU5JkW9fZyX") ;;
    }
...
/*
    ...
    public void updateComment(JSONObject json) throws SQLException {
        if (!Production)
            return;
        int id = (int) json.get("Waybill_number");
        PreparedStatement stmt = exec.getConn().prepareStatement("UPDATE weighings set comment = ?  WHERE id = ?");
        stmt.setString(1, (String) json.get("Comment"));
        stmt.setLong(2, id);
        stmt.executeUpdate();
    };

    public void updateweighing_items(JSONObject  json) throws SQLException {
        if (!Production)
            return;
        var id = getId(json);
